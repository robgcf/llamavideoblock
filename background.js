/**
 * LlamaAutoPlayBlock — background service worker.
 *
...
async function refreshIcon() {
  try {
    const enabled = await LlamaAutoPlayBlockStore.isEnabled();
    await chrome.action.setIcon({ path: enabled ? COLOUR_ICONS : GRAY_ICONS });
  } catch (error) {
    console.error('[LlamaAutoPlayBlock] Failed to refresh toolbar icon:', error);
  }
}
...
async function refreshBadge(tabId, url) {
  try {
    const { enabled, whitelist } = await LlamaAutoPlayBlockStore.getSettings();
    const hostname = LlamaAutoPlayBlockDomain.fromUrl(url);
    const whitelisted = enabled && LlamaAutoPlayBlockDomain.isWhitelisted(whitelist, hostname);
    ...
    if (!isMissingTabError(error)) {
      console.error('[LlamaAutoPlayBlock] Failed to refresh badge:', error);
    }
  }
}
...
async function refreshAllTabs() {
  try {
    ...
    console.error('[LlamaAutoPlayBlock] Failed to refresh tabs:', error);
  }
}
...
function queueCountWrite(write) {
  countWrites = countWrites
    .then(write)
    .catch((error) => console.error('[LlamaAutoPlayBlock] Failed to record blocked count:', error));
}
...
chrome.runtime.onMessage.addListener((message, sender) => {
  ...
  if (message.type === 'blockedCount') {
    ...
    queueCountWrite(() => LlamaAutoPlayBlockStore.setFrameCount(tabId, frameId, count));
    return false;
  }
  ...
  if (message.type === 'debugLines' && Array.isArray(message.lines)) {
    ...
    queueCountWrite(() => LlamaAutoPlayBlockStore.appendDebugLines(tabId, message.lines));
  }
  return false;
});
...
chrome.tabs.onActivated.addListener(({ tabId }) => {
  void (async () => {
    try {
      ...
      if (!isMissingTabError(error)) {
        console.error('[LlamaAutoPlayBlock] Failed to handle tab activation:', error);
      }
    }
  })();
});
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (changeInfo.status === 'loading') {
    queueCountWrite(() => LlamaAutoPlayBlockStore.clearTabCount(tabId));
    queueCountWrite(() => LlamaAutoPlayBlockStore.clearDebugLines(tabId));
  }
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  const affectsBlocking =
    (areaName === 'sync' && LlamaAutoPlayBlockStore.WHITELIST_KEY in changes) ||
    (areaName === 'local' && LlamaAutoPlayBlockStore.ENABLED_KEY in changes);
